...
def main():
    ...
    season = '2023-24'  # Ensure the format is compatible with data-fetching functions
    ...
    if KG is None or not KG.nodes:
        logging.error("Knowledge graph building failed. Exiting the program.")
        sys.exit(1)

    # Proceed with data preprocessing and model training
    best_reg_pipeline, best_clf_pipeline = build_and_train_models(all_game_logs, players_df)
